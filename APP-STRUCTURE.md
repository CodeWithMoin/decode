...
│   │
│   ├── project/
│   │   ├── ProjectShell.tsx          ✅ header · gated stages · stage slot
│   │   ├── ProjectChatDock.tsx       ✅ persistent 344px Production room column
│   │   ├── Inspector.tsx             ✅ 344px scene properties panel, Edit only
│   │   ├── ExportModal.tsx           ✅ Edit-only export settings and progress
│   │   ├── stages/
│   │   │   ├── Understanding.tsx     ✅ Producer
│   │   │   ├── TeachingPlan.tsx      ✅ Educator
│   │   │   ├── Script.tsx            ✅ Script Writer
│   │   │   └── Edit.tsx              ✅ Motion Designer + Editor
│   │   ├── canvas/SceneVisual.tsx    ✅ 8 renderers + 2 alternates, frame-driven
│   │   └── timeline/Timeline.tsx     ✅ ruler · scene tracks · editing · shuttle
│   │
│   ├── player/
│   │   ├── DecodeComposition.tsx     ✅ Decode scene sequencing and frame markup
│   │   └── DecodePlayer.tsx          ✅ Decode-owned @remotion/player boundary
│   │
│   ├── crew/
│   │   ├── HandoffCard.tsx           ✅ the approve gate
│   │   └── SpecialistNote.tsx        ✅ the A/B option card (scenes 3 & 5)
│   │
│   ├── producer/ProducerDrawer.tsx   ✅ persistent in projects, overlay when narrow
│   │
│   └── ui/
...
### Project shell ✅
Header: back pill · title · draft pill · derived `runtime · n scenes` · four
centred stage pills · Edit-only Export action. The Production room occupies a
344px left column at laptop widths. The 344px right inspector appears only in
Edit.
...
script`, and approving script jumps the level to 5, opening Edit. A locked stage
opens the Production room and gets
...
| Edit | Motion Designer + Editor | Remotion Preview + Inspector + Timeline + export modal | ✅ |

Export is a compact header action in Edit, matching the stage-pill height and
dark treatment. Its Decode-owned modal configures
...
- **`player/DecodeComposition.tsx`** — the 1920×1080, 24fps Remotion markup.
  Scenes are `<Sequence>` blocks driven by Decode scene durations and props.
...
- **`Timeline.tsx`** — transport and J/K/L shuttle, adaptive frame-accurate
  ...
  add, confirmed remove and project undo/redo via Cmd/Ctrl shortcuts.

---

## 5. Production room ✅

Project-scoped conversation surface. At laptop widths it is a persistent 344px
left column across all four stages; below 1024px it becomes an on-demand overlay.
The surface is opaque `#FCFCFB` and remains above sticky project chrome.
...
Access: `⌘J`/`Ctrl+J`; `Esc` closes the narrow overlay. Dashboard and New Decode
retain their own entry points, while project pages keep the room visible without
...
- **The crew.** Five named specialists from `lib/crew.ts`. Never "Assistant" or
...
- **Edit below 1280px** — the production editor is laptop-only; do not silently
  collapse it into a compromised mobile timeline.

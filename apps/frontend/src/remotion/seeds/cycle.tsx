...
import Scene from "@decode/seeds/cycle";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "Training is a loop. Forward pass, measure the loss, backward pass, update the weights, then around again.";
const STEP = 0.3;
...
export const CYCLE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 2.6) * 30);
const CUES = new Set(["loop", "cycle"]);

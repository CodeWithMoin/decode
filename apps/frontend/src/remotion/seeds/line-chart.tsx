...
import Scene from "@decode/seeds/line-chart";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "As training went on, accuracy rose sharply at first, then leveled off near ninety-five percent.";
const STEP = 0.3;
...
export const LINE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.8) * 30);
const CUES = new Set(["training", "rose", "leveled"]);

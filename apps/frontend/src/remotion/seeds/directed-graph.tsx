...
import Scene from "@decode/seeds/directed-graph";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "A graph is just nodes joined by arrows. Here node A feeds both B and C, and they both flow into D.";
const STEP = 0.3;
...
export const DIGRAPH_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 2.4) * 30);
const CUES = new Set(["graph", "nodes"]);

export function DirectedGraphPreview() {

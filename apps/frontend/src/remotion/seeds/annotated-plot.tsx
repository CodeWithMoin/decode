...
import Scene from "@decode/seeds/annotated-plot";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT = "The loss falls as we tune the parameter, and gradient descent settles at the minimum.";
const STEP = 0.3;
...
export const ANNOTATED_PLOT_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.8) * 30);
const CUES = new Set<string>(["loss", "minimum"]);

export function AnnotatedPlotPreview() {

...
import Scene from "@decode/seeds/encode";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "A computer stores the letter A as a byte. In binary, that is zero one zero zero zero zero zero one.";
const STEP = 0.34;
...
export const ENCODE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set(["binary", "byte"]);

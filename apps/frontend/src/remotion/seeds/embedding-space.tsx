...
import Scene from "@decode/seeds/embedding-space";
type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");
const SCRIPT = "An embedding places words in space, so similar meanings land together.";
const STEP = 0.34;
...
export const EMBEDDING_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.5) * 30);
const CUES = new Set(["embedding", "together"]);
export function EmbeddingSpacePreview() {

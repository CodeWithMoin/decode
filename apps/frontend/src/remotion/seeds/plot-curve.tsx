...
import Scene from "@decode/seeds/plot-curve";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "Every weight setting has a loss. Gradient descent starts high on the curve and walks downhill, step by step, until it settles at the minimum.";
const STEP = 0.3;
...
export const PLOT_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);

const CUES = new Set(["loss", "high", "downhill", "minimum"]);

export function PlotCurvePreview() {

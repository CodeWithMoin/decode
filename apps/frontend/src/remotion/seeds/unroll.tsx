...
import Scene from "@decode/seeds/unroll";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT = "An RNN is the same cell unrolled in time, passing a hidden state from step to step.";
const STEP = 0.3;
...
export const UNROLL_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set<string>(["unrolled"]);

export function UnrollPreview() {

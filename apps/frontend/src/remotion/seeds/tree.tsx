...
import Scene from "@decode/seeds/tree";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "Machine learning splits into two families. Supervised learning uses labeled data, for classification and regression. Unsupervised learning finds structure on its own, like clustering.";
const STEP = 0.3;
...
export const TREE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set(["learning", "supervised", "unsupervised"]);

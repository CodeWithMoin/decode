...
import Scene from "@decode/seeds/equation";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "A perceptron is just this. y equals w times x plus b, a weighted input plus a bias.";
const STEP = 0.3;
...
export const EQUATION_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set(["perceptron"]);

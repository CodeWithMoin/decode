...
import Scene from "@decode/seeds/loss-landscape";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT = "Gradient descent walks downhill across the contours until it reaches the minimum.";
const STEP = 0.3;
...
export const LOSS_LANDSCAPE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set<string>(["descent"]);

export function LossLandscapePreview() {

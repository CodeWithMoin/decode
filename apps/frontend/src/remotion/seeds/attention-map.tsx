...
import Scene from "@decode/seeds/attention-map";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT = "Each cell shows how much one token attends to another; a brighter cell means a higher attention weight.";
const STEP = 0.3;
...
export const ATTENTION_MAP_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set<string>(["attention"]);

export function AttentionMapPreview() {

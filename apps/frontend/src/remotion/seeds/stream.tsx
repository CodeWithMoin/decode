...
import Scene from "@decode/seeds/stream";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "The producer streams data to the consumer, one message at a time, without stopping.";
const STEP = 0.34;
...
export const STREAM_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.8) * 30);
const CUES = new Set(["streams", "data"]);

...
import Scene from "@decode/seeds/bar-chart";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "We compared three optimizers on the same task. Plain SGD reached seventy-one percent. Adding momentum pushed it to eighty-four. Adam climbed highest, to ninety-two percent.";
const STEP = 0.3;
...
export const BAR_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set(["sgd", "momentum", "adam"]);

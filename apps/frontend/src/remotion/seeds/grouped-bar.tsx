...
import Scene from "@decode/seeds/grouped-bar";
type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");
const SCRIPT = "We compared two models. The new model beats the old across every dataset.";
const STEP = 0.34;
...
export const GROUPEDBAR_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.7) * 30);
const CUES = new Set(["compared", "models"]);
export function GroupedBarPreview() {

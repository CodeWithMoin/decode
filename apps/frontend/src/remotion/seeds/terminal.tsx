...
import Scene from "@decode/seeds/terminal";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT = "We install the library, then run training and watch the loss drop each epoch.";
const STEP = 0.3;
...
export const TERMINAL_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set<string>(["install"]);

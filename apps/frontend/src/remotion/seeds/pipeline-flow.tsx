...
import Scene from "@decode/seeds/pipeline-flow";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "Every request travels through the pipeline. It leaves the client, reaches the server, and lands in the database.";
const STEP = 0.32;
...
export const PIPELINE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 2.2) * 30);
const CUES = new Set(["request", "travels", "pipeline"]);

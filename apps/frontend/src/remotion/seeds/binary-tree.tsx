...
import Scene from "@decode/seeds/binary-tree";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "A binary search tree keeps smaller values on the left and larger on the right, so every lookup follows one path down.";
const STEP = 0.3;
...
export const BINTREE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set(["tree", "left", "right"]);

export function BinaryTreePreview() {

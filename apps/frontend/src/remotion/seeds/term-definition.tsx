...
import Scene from "@decode/seeds/term-definition";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "Overfitting is when a model memorises its training data instead of learning the pattern, so it fails on anything new.";
const STEP = 0.3;
...
export const TERMDEF_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.2) * 30);
const CUES = new Set(["overfitting"]);

export function TermDefPreview() {

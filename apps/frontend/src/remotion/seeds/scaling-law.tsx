...
import Scene from "@decode/seeds/scaling-law";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT = "As we scale compute the loss follows a power law, a straight line on log-log axes.";
const STEP = 0.3;
...
export const SCALING_LAW_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set<string>(["compute"]);

export function ScalingLawPreview() {

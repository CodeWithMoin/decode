...
import Scene from "@decode/seeds/number-line";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "A probability is a number between zero and one. Here the probability is zero point seven, most of the way along the line.";
const STEP = 0.3;
...
export const NUMLINE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set(["probability", "line"]);

export function NumberLinePreview() {

...
import Scene from "@decode/seeds/array";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "An array stores its items in a row, each at a numbered index. To read one, jump straight to its index, no scanning.";
const STEP = 0.3;
...
export const ARRAY_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.4) * 30);
const CUES = new Set(["array", "index"]);

...
import Scene from "@decode/seeds/checklist";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT = "The training recipe is four steps that we run in order for every model.";
const STEP = 0.3;
...
export const CHECKLIST_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set<string>(["steps"]);

...
import Scene from "@decode/seeds/list-reveal";
type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");
const SCRIPT = "There are three ways to scale. Scale horizontally, scale vertically, or cache what you can.";
const STEP = 0.34;
...
export const LISTREVEAL_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set(["horizontally", "vertically", "cache"]);
export function ListRevealPreview() {

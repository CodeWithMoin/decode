...
import Scene from "@decode/seeds/two-pointer";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "Two pointers start at the ends. If the sum is too big, move hi left. Too small, move lo right, until they hit the target.";
const STEP = 0.36;
...
export const TWOPTR_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set(["pointers", "ends"]);

export function TwoPointerPreview() {

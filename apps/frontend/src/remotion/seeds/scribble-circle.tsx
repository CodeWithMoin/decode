...
import Scene from "@decode/seeds/scribble-circle";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT = "The failure mode to watch for in training is overfitting to the data.";
const STEP = 0.3;
...
export const SCRIBBLE_CIRCLE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set<string>(["overfitting"]);

export function ScribbleCirclePreview() {

...
import Scene from "@decode/seeds/vector";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "A vector is an arrow from the origin. Here vector a points one way, and vector b points another.";
const STEP = 0.3;
...
export const VECTOR_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.4) * 30);
const CUES = new Set(["vector"]);

...
import Scene from "@decode/seeds/crud";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "A database supports four operations. Select finds a row, insert adds a new one, update changes a field, and delete removes a row.";
const STEP = 0.35;
...
export const CRUD_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set(["select", "insert", "update", "delete"]);

export function CrudPreview() {

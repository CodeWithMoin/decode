...
import Scene from "@decode/seeds/state-machine";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "A state machine moves between states. From Idle it can start running, tick along, finish to Done, then reset.";
const STEP = 0.3;
...
export const STATEMACHINE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 2.6) * 30);
const CUES = new Set(["state", "machine"]);

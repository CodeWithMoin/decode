...
import Scene from "@decode/seeds/split-compare";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT = "The baseline reaches seventy-eight percent, while ours reaches ninety-two.";
const STEP = 0.3;
...
export const SPLIT_COMPARE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.8) * 30);
const CUES = new Set<string>(["baseline"]);

export function SplitComparePreview() {

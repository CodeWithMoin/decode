...
import Scene from "@decode/seeds/scatter-plot";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "Each point is one data example. Plotted by two features, the two classes separate into clear clusters.";
const STEP = 0.3;
...
export const SCATTER_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.4) * 30);
const CUES = new Set(["point", "example", "classes", "clusters"]);

export function ScatterPlotPreview() {

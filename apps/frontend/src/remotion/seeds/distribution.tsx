...
import Scene from "@decode/seeds/distribution";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "Heights follow a normal distribution. Most values cluster near the average, with far fewer out at the extremes.";
const STEP = 0.3;
...
export const DIST_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set(["distribution", "cluster", "average"]);

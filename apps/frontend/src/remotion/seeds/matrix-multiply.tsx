...
import Scene from "@decode/seeds/matrix-multiply";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "To multiply matrix A by matrix B, each output cell is a row of A dotted with a column of B.";
const STEP = 0.3;
...
export const MATRIX_MULTIPLY_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 2.6) * 30);
const CUES = new Set<string>(["multiply"]);

export function MatrixMultiplyPreview() {

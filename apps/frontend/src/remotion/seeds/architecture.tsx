...
import Scene from "@decode/seeds/architecture";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "The input embedding and positional encoding feed a stack: multi-head attention, add and norm, feed forward, add and norm, then linear and softmax.";
const STEP = 0.3;
...
export const ARCHITECTURE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.4) * 30);
const CUES = new Set<string>(["embedding"]);

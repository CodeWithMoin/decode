...
import Scene from "@decode/seeds/derivation";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT = "We start from the loss and derive the gradient step by step using the chain rule.";
const STEP = 0.3;
...
export const DERIVATION_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set<string>(["loss"]);

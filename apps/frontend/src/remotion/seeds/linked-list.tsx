...
import Scene from "@decode/seeds/linked-list";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "A current pointer walks the list, following each next arrow, until it reaches null.";
const STEP = 0.36;
...
export const LINKEDLIST_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set(["current", "walk"]);

export function LinkedListPreview() {

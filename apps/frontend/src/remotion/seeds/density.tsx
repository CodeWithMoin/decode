...
import Scene from "@decode/seeds/density";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT = "This density peaks at the mean, and the shaded band is one standard deviation of the distribution.";
const STEP = 0.3;
...
export const DENSITY_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set<string>(["distribution"]);

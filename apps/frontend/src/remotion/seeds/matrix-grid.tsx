...
import Scene from "@decode/seeds/matrix-grid";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "Attention scores every pair of tokens. Each cell is how strongly one word attends to another. Brighter means stronger. Here, the word sat attends most to cat.";
const STEP = 0.3;
...
export const MATRIX_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);

const CUES = new Set(["scores", "brighter", "most", "sat", "cat"]);

export function MatrixGridPreview() {

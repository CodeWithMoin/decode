...
import Scene from "@decode/seeds/timeline";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "The timeline began with GPT-2, then GPT-3, then GPT-4, and now GPT-5, each bigger than the last.";
const STEP = 0.3;
...
export const TIMELINE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 2.0) * 30);
const CUES = new Set(["timeline", "began"]);

...
import Scene from "@decode/seeds/self-attention";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT = "In self-attention every token attends to the others; a thicker arc means a stronger weight.";
const STEP = 0.3;
...
export const SELF_ATTENTION_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set<string>(["attends"]);

export function SelfAttentionPreview() {

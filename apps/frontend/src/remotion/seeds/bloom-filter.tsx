...
import Scene from "@decode/seeds/bloom-filter";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "A bloom filter hashes each item to a few positions, and flips those bits to one, so it can later check if an item might be present.";
const STEP = 0.3;
...
export const BLOOM_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.6) * 30);
const CUES = new Set(["bloom", "hash", "bits"]);

export function BloomFilterPreview() {

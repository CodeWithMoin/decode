...
import Scene from "@decode/seeds/marker-highlight";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT = "What we really want from a model is not memorization but generalization.";
const STEP = 0.3;
...
export const MARKER_HIGHLIGHT_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.4) * 30);
const CUES = new Set<string>(["generalization"]);

export function MarkerHighlightPreview() {

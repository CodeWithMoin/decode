...
import Scene from "@decode/seeds/queue";

type Word = { word: string; startInSeconds: number; endInSeconds: number };
const norm = (s: string) => s.toLowerCase().replace(/[^a-z0-9]/g, "");

const SCRIPT =
  "A queue is first in, first out. New items join at the back, and the oldest one leaves from the front.";
const STEP = 0.3;
...
export const QUEUE_PREVIEW_DURATION = Math.round((WORDS.length * STEP + 1.5) * 30);
const CUES = new Set(["queue", "front", "back"]);

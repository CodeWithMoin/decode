    ...
    >
      <div className="grid grid-cols-1 gap-x-[clamp(40px,6vw,88px)] gap-y-10 lg:grid-cols-[minmax(260px,0.7fr)_minmax(0,1.3fr)]">
        <div>
          ...
          >
            Three decisions,
            <span className="text-t8"> and Decode does the rest.</span>
          </h2>
          <p
            data-reveal
            className="pretty m-0 mt-6 max-w-[42ch] text-[15px] leading-[1.7] text-ink-2"
          >
            You bring the source, change what you want, and approve the video.
            ...
            return (
              ...
              >
                <div className="studio-surface grid grid-cols-[26px_minmax(0,1fr)_36px] items-center gap-x-3 gap-y-1 px-4 py-5 transition-[transform,box-shadow] duration-[var(--t-fast)] ease-decode group-hover:-translate-y-px group-hover:shadow-md sm:grid-cols-[32px_150px_minmax(0,1fr)_40px] sm:gap-x-4 sm:px-5">
                  ...
                  <span className="col-start-3 row-span-2 row-start-1 grid h-9 w-9 place-items-center rounded-full bg-sunken text-t6 transition-[background-color,color,transform] duration-[var(--t-fast)] ease-decode group-hover:-translate-y-px group-hover:bg-ink group-hover:text-white sm:col-start-4 sm:row-span-1">

...
export function ConnectedProcessing({ projectId, jobId }: { projectId: string; jobId: string }) {
  ...
  // Follow the chain when the project is continuous.
  //
  // With auto_continue on, a finished stage has already started the next one, so
  // stopping here on a "Review →" gate is the wizard we are collapsing. Forward
  // to wherever the production now is — the next running job's progress, or the
  // cutting room once scenes exist. projectRoute owns that decision. When the
  // snapshot hasn't caught up to the chained job yet it still points back here;
  // nudge a refresh rather than bouncing to the stage this job just finished.
  useEffect(() => {
    if (job?.status !== "succeeded" || !studio?.project.auto_continue) return;

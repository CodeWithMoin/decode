...
from ..providers.producer import SourceInput, evaluator, producer
    ...
    try:
        async with SessionLocal() as session:
            ...
            await emit(
                ...
                job_id=job.id,
                run_id=run.id,
                data={"step": "generating_brief", "source_count": len(sources)},
            )
            settings = get_settings()
            current_producer, current_evaluator = producer(settings), evaluator(settings)
            brief = await current_producer.generate(intent, sources)
            project = await session.scalar(
                select(Project)
            ...
            version = await publish_version(
                ...
                rationale=f"Production Brief generated by {current_producer.identifier}.",
                parents=parents,
            )
            await emit(
                session,
            ...
            decision, checks, summary = current_evaluator.evaluate(brief)
            session.add(
                Evaluation(
                    artifact_version_id=version.id,
                    evaluator=current_evaluator.identifier,
                    decision=decision,
                    checks=checks,
                    summary=summary,
                )
            ...
            session.add_all(
                [
                    UsageRecord(
                        ...
                        run_id=run.id,
                        artifact_version_id=version.id,
                        provider=settings.producer,
                        operation="production_brief_generation",
                        model=current_producer.identifier,
                        duration_ms=0,
                    ...
                    UsageRecord(
                        ...
                        model=current_evaluator.identifier,
                        duration_ms=0,
                        estimated_cost_usd=Decimal("0"),
                    ),
                ]
